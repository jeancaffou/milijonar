    ...
    const body = `${breadcrumbs(lang, [
      ...
      ${questions
        .map((question) => {
          ...
          const primaryEvidence = question.displayPrimaryEvidence || question.primaryEvidence || questionEvidence[0];
          const supplementalEvidence = Array.isArray(question.displaySupplementalEvidence)
            ? question.displaySupplementalEvidence
            : questionEvidence.filter((item) => item.url !== primaryEvidence?.url);
