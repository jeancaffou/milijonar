...
function auditedProfileImage(person, contestantRuns, logicalRuns, profileAuditByKey) {
  if (!person || !profileAuditByKey) return null;
  const evidenceBySourcePath = new Map();
  for (const run of [...(contestantRuns || []), ...(logicalRuns || [])]) {
    for (const item of run?.evidence || []) {
      if (item?.sourcePath) evidenceBySourcePath.set(item.sourcePath, item);
    }
  }
  for (const appearance of person.lineupAppearances || []) {
    for (const item of appearance.evidence || []) {
      if (item?.sourcePath) evidenceBySourcePath.set(item.sourcePath, item);
    }
    const key = `${appearance.episodeKey}:${person.name}`;
    const audit = profileAuditByKey[key];
    if (!audit || !["verified", "accepted"].includes(String(audit.status || "").toLowerCase())) continue;
    const item = evidenceBySourcePath.get(String(audit.primary || "").trim());
    if (item) return { item, source: "audited-profile", audit };
  }
  return null;
}

export function selectContestantProfileImage(person, contestantRuns, logicalRuns, episodeByKey = {}, profileAuditByKey = {}) {
  const audited = auditedProfileImage(person, contestantRuns, logicalRuns, profileAuditByKey);
...
const strongProfileSources = new Set(["contestant-introduction", "hot-seat"]);

export function selectRunProfileImage(person, contestantRuns = [], run, logicalRuns = [], episodeByKey = {}, profileAuditByKey = {}) {
  if (!person || !run) return null;

  const episodeKeys = new Set(run.episodes || []);
  const scopedPerson = {
    ...person,
    lineupAppearances: (person.lineupAppearances || []).filter((appearance) => episodeKeys.has(appearance.episodeKey)),
    questions: run.questions || [],
    episodes: (person.episodes || []).filter((episode) => episodeKeys.has(episode.key)),
  };
  const scopedRows = contestantRuns.filter((row) => episodeKeys.has(row.episodeKey));
  const local = selectContestantProfileImage(scopedPerson, scopedRows, [run], episodeByKey, profileAuditByKey);
  if (strongProfileSources.has(local?.source)) return local;

  const global = selectContestantProfileImage(person, contestantRuns, logicalRuns, episodeByKey, profileAuditByKey);
  if (strongProfileSources.has(global?.source)) return global;
