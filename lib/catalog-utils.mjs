...
function auditedQuestionEvidence(evidence, audit) {
  ...
  // The catalogue's question-image contract is now strict: a verified row
  // must have a native settled green-correct reveal. Historical no-green
  // exceptions remain useful audit history, but they must never become a
  // public question image again.
  if (audit.greenSettled !== true || audit.noGreenException === true) {
    return null;
  }
  const auditedPrimaryPath = String(audit.primary || "").trim();
  // Season 1's public contract is strict: the first CSV evidence path is the
  // default image. The ledger must explicitly say whether that image is a
  // settled green reveal or a GPT-reviewed no-green exception; an omitted or
  // contradictory flag must never silently become a default image.
  if (Number(audit.season) === 1) {
    if (typeof audit.greenSettled !== "boolean"
      || typeof audit.noGreenException !== "boolean"
      || audit.noGreenException === audit.greenSettled) {
      throw new Error(
        `S01 audit must explicitly distinguish green reveal from no-green exception (row ${audit.rowIndex})`,
      );
    }
    if (auditedPrimaryPath !== String(evidence[0]?.sourcePath || "").trim()) {
      throw new Error(
        `S01 audited primary must be the first CSV evidence path (row ${audit.rowIndex}): `
        + `${auditedPrimaryPath || "<missing>"} != ${evidence[0]?.sourcePath || "<missing>"}`,
      );
    }
  }
  const bySourcePath = new Map(evidence.map((item) => [item.sourcePath, item]));
  const primary = bySourcePath.get(auditedPrimaryPath);
...
export function selectQuestionEvidence(items, { lifelineKeys = [], notes = "", audit = null } = {}) {
  ...
  // Once a row has an audit record, do not fall back to the historical
  // filename heuristic. That heuristic is exactly how yellow/orange
  // contestant-selection frames previously leaked into the public viewer.
  if (audit) {
    return {
      primary: undefined,
      supplemental: [],
      mode: "strict-green-pending",
      hasCompleteBoard: false,
      hasExplicitCorrectOutcome: false,
      auditVerified: false,
    };
